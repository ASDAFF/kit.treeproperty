...
$(document).ready(function () {
    $(document).on('change', '.kit_treeproperty select[name=type]', function () {
        var val = $(this).val();
        ...
        'contextmenu': {
            items: function (o, cb) {
                ...
                if (o.type == 'property') {
                    ...
                    var text = $.ajax({
                        method: "POST",
                        url: "/bitrix/ajax/kit_treeproperty_ajax.php",
                        async: false,
                        dataType: 'json',
                        data: {
                            action: 'get_products',
                                ...
                                "action": function(){
                                    $.ajax({
                                        ...
                                        data: {
                                            ...
                                            iblock: $("#kit_treeproperty_form select[name=block]").val(),
                                            property_id: o.id,
                                        },
                                        success: function(data){
                                            if(data['type_ok']){
                    ...
                    if (o.icon == 'file not_inherited') {
                        menu.remove = {
                            ...
                            "action": function (data) {
                                var inst = $('#kit_treeproperty_js').jstree(true),
    ...
    $(document).on('change', '.kit_treeproperty select[name=block]', function () {
        //reload
        $('#kit_treeproperty_js').jstree(true).destroy();
        $('#kit_treeproperty_js').jstree(jstree);
...
function bindEvents() {
    ...
    $("#kit_treeproperty_js").bind("move_node.jstree", function (e, data) {
        if (data.old_parent != data.parent) {
            $.ajax({
                ...
                if (data.parent == '#' || data.old_parent == '#') {
                    $('#kit_treeproperty_js').jstree(true).refresh();
                } else {
                    anode = $("#kit_treeproperty_js").jstree(true).get_node(data.parent);
                    anode.state.loaded = false;
                    $("#kit_treeproperty_js").jstree(true).refresh_node(anode);
                    anode = $("#kit_treeproperty_js").jstree(true).get_node(data.old_parent);
                    anode.state.loaded = false;
                    $("#kit_treeproperty_js").jstree(true).refresh_node(anode);
                }
            });
    ...
    $("#kit_treeproperty_js").bind("copy_node.jstree", function (e, data) {
    ...
    $("#kit_treeproperty_js").bind("delete_node.jstree", function (e, data) {
